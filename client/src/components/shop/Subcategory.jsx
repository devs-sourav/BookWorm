...
const Subcategory = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Updated API endpoint for subcategory products
      const response = await axios.get(`${baseApi}/subCategory/${subcategoryId}/products`, { params });
      const fetchedProducts = response.data.data.products || [];
      
      // Always reset subcategory info first to ensure it changes
      setSubcategoryInfo(null);
      
      // Get subcategory info if available (try to get from first product or response)
      if (fetchedProducts.length > 0 && fetchedProducts[0].subCategory) {
        setSubcategoryInfo(fetchedProducts[0].subCategory);
      } else if (response.data.data.subCategory) {
        setSubcategoryInfo(response.data.data.subCategory);
      }
      ...
      // Filter by color if selected (for books, this might need adjustment based on your needs)
      if (selectedColor) {
        processedProducts = processedProducts.filter(product => {
          // Since books might not have color variants, you can adjust this logic
          // or remove color filtering for book subcategories
      ...
      setError("Failed to load subcategory products. Please try again later.");
  ...
  useEffect(() => {
    if (baseApi && subcategoryId) {
      // Reset states when subcategory changes
      setAllProduct([]);
      setFilteredProducts([]);
      setCurrentPage(1);
      setSearchQuery("");
  ...
  }, [baseApi, subcategoryId, minPrice, maxPrice, selectedSortOption, selectedColor]);

  useEffect(() => {
    // Reset subcategory info when subcategoryId changes
    setSubcategoryInfo(null);
    dispatch(resetColor());
            ...
            <div>
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-800 flex items-center gap-2">
                ...
                <span className="bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
                  {subcategoryInfo?.title || 'Subcategory Products'}
                </span>
              </h1>
              <p className="text-gray-600 mt-1">
                {subcategoryInfo?.description || 'Explore our curated collection of products in this subcategory'}
              </p>
            </div>
          ...
          {!loading && filteredProducts.length > 0 && (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg p-3 text-center">
                ...
                <div className="text-2xl font-bold text-purple-700">{weeklyStats.thisWeek}</div>
              </div>
              <div className="bg-gradient-to-r from-pink-50 to-rose-50 rounded-lg p-3 text-center">
                ...
                <div className="text-2xl font-bold text-pink-700">{weeklyStats.lastWeek}</div>
              </div>
      ...
      <div className="p-6">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-12 text-center">
            ...
            <p className="text-gray-600 mb-4 max-w-md">
              {searchQuery 
                ? `We couldn't find any products matching "${searchQuery}" in this subcategory. Try different search terms.`
                : "No products are available in this subcategory at the moment. Check back soon for new arrivals!"
              }
            </p>
            {searchQuery && (
              <button
              ...
              return (
                ...
                >
                  ...
                  <ProductItem
                    ...
                    image={item.photos}
                    id={item._id}
                    subtitle={item.author?.name || item.brand?.title}
                    title={item.title}
                    ...
                    classItem={viewMode === "list" ? "w-full" : "w-full"}
                    viewMode={viewMode}
                  />
                </div>
              );
        ...
        {paginatedProducts.length > 0 && totalPages > 1 && (
          <PaginationControls
            ...
            subcategoryName={subcategoryInfo?.title}
          />
        )}
      </div>
    ...
    <div className="mt-12 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
        <div className="text-sm text-gray-600">
          ...
          {subcategoryName && (
            <span className="text-purple-600 font-medium"> in {subcategoryName}</span>
          )}
        </div>
        <div className="text-sm text-gray-600">
