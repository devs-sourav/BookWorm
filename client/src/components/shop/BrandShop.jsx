...
import { FaChevronLeft, FaChevronRight, FaSearch, FaTimes, FaStar, FaClock } from "react-icons/fa";
import { FiGrid, FiList, FiRefreshCw, FiTrendingUp, FiTag } from "react-icons/fi";
...
const BrandShop = () => {
  ...
  useEffect(() => {
    if (filteredProducts.length > 0) {
      ...
      const onSaleProducts = filteredProducts.filter(product => {
        return product.discountValue > 0 && product.salePrice < product.price;
      });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Updated API endpoint for brand products
      const response = await axios.get(`${baseApi}/brand/${brandId}/products`, { params });
      const fetchedProducts = response.data.data.products || [];
      
      // Always reset publisher info first to ensure it changes
      setPublisherInfo(null);
      
      // Get publisher info if available (try to get from first product or response)
      if (fetchedProducts.length > 0 && fetchedProducts[0].brand) {
        setPublisherInfo(fetchedProducts[0].brand);
      } else if (response.data.data.brand) {
        setPublisherInfo(response.data.data.brand);
      }
      ...
      // Filter by color if selected (for books, this might not be applicable)
      if (selectedColor) {
        processedProducts = processedProducts.filter(product => {
          // Since books might not have color variants, you can adjust this logic
          // or remove color filtering for book brands
          return product.colorName?.toLowerCase() === selectedColor.toLowerCase();
        });
      }
      ...
      setError("Failed to load publisher products. Please try again later.");
  ...
  }, [baseApi, brandId, minPrice, maxPrice, selectedSortOption, selectedColor]);

  useEffect(() => {
    // Reset publisher info when brandId changes
    setPublisherInfo(null);
    dispatch(resetColor());
  }, [brandId, dispatch]);
  ...
  // Loading skeleton component with fixed height
  const ProductSkeleton = () => (
    <div className="animate-pulse bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden h-[350px] flex flex-col">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-4">
            <div>
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-800 flex items-center gap-2">
                <MdPublish className="text-orange-500" />
                <span className="bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text text-transparent">
                  {publisherInfo?.title || 'Publisher'} Books
                </span>
              </h1>
              <p className="text-gray-600 mt-1">
                Discover amazing books from this renowned publisher
              </p>
            </div>
          ...
          {publisherInfo && (
            <div className="bg-gradient-to-r from-orange-50 to-red-50 rounded-lg p-4 mb-4 border border-orange-200">
              <div className="flex items-center gap-4">
                {publisherInfo.photo && (
                  <img
                    src={publisherInfo.photo}
                    alt={publisherInfo.title}
                    className="w-16 h-16 rounded-lg object-cover border-2 border-white shadow-md"
                  />
                )}
                <div>
                  <h2 className="text-lg font-semibold text-gray-800">{publisherInfo.title}</h2>
                  <p className="text-sm text-gray-600">Trusted Publisher</p>
                </div>
              </div>
            </div>
          )}
          ...
          {!loading && filteredProducts.length > 0 && (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              <div className="bg-gradient-to-r from-orange-50 to-red-50 rounded-lg p-3 text-center">
                ...
                <div className="text-2xl font-bold text-orange-700">{weeklyStats.thisWeek}</div>
              </div>
              <div className="bg-gradient-to-r from-red-50 to-pink-50 rounded-lg p-3 text-center">
                ...
                <div className="text-2xl font-bold text-red-700">{weeklyStats.lastWeek}</div>
              </div>
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-3 text-center">
                ...
                <div className="text-2xl font-bold text-green-700">{weeklyStats.trending}</div>
              </div>
              <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg p-3 text-center">
                ...
                <div className="text-2xl font-bold text-yellow-700">{weeklyStats.onSale}</div>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              <div className="bg-orange-50 text-orange-700 px-3 py-1 rounded-full text-sm font-medium">
                {loading ? "Loading..." : `${totalProducts} Books Found`}
              </div>
              {searchQuery && (
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-12 text-center">
            <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center mb-4">
              <MdPublish className="text-gray-400 text-3xl" />
            </div>
            ...
            <p className="text-gray-600 mb-4 max-w-md">
              {searchQuery 
                ? `We couldn't find any books matching "${searchQuery}" from this publisher. Try different search terms.`
                : "No books are available from this publisher at the moment. Check back soon for new releases!"
              }
            </p>
            {searchQuery && (
              <button
          ...
          <div className={`grid gap-5 ${
            ...
            {paginatedProducts.map((item, index) => {
              ...
              const isOnSale = item.discountValue > 0 && item.salePrice < item.price;
              ...
              return (
                ...
                >
                  {/* New Badge */}
                  {daysAgo <= 7 && (
                    <div className="absolute top-2 left-2 z-10 bg-gradient-to-r from-green-500 to-emerald-500 text-white px-2 py-1 rounded-full text-xs font-bold shadow-lg flex items-center gap-1">
                  ...
                  {/* Sale Badge */}
                  {isOnSale && daysAgo > 7 && (
                    <div className="absolute top-2 left-2 z-10 bg-gradient-to-r from-red-500 to-pink-500 text-white px-2 py-1 rounded-full text-xs font-bold shadow-lg flex items-center gap-1">
                      <FiTag className="text-xs" />
                      {item.discountType === 'percent' ? `${item.discountValue}% OFF` : 'SALE'}
                    </div>
                  )}
                  
                  {/* Bestseller Badge for high visit count */}
                  {item.visitCount > 100 && !isOnSale && daysAgo > 7 && (
                    <div className="absolute top-2 left-2 z-10 bg-gradient-to-r from-yellow-500 to-orange-500 text-white px-2 py-1 rounded-full text-xs font-bold shadow-lg flex items-center gap-1">
                      <FaStar className="text-xs" />
                      BESTSELLER
                    </div>
                  )}
                  
                  <div className={viewMode === "grid" ? "h-full" : "h-auto"}>
                    <ProductItem
                      product={item}
                      image={item.photos}
                      id={item._id}
                      subtitle={item.author?.name || publisherInfo?.title}
                      title={item.title}
                      ...
                      classItem={viewMode === "list" ? "w-full" : "w-full h-full"}
                      viewMode={viewMode}
                      fixedHeight={viewMode === "grid" ? "350px" : "auto"}
                    />
                  </div>
                </div>
              );
    ...
    <div className="mt-12 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
        <div className="text-sm text-gray-600">
          ...
          <span className="font-semibold text-gray-800">{totalProducts}</span> books
          {publisherName && (
            <span className="text-orange-600 font-medium"> from {publisherName}</span>
          )}
        </div>
        <div className="text-sm text-gray-600">
