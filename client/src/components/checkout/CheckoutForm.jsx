  ...
  const getCZone = async (id) => {
    try {
      const response = await axios.post(
        `http://localhost:8000/api/v1/pathaoLocation/city/${id}/zones`
      );
  ...
  const getCArea = async (id) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8000/api/v1/pathaoLocation/zone/${id}/area-list`
      );
  ...
  const handleCouponCode = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8000/api/v1/coupon/${formData.couponCode}`
      );
    ...
    try {
      ...
      if (formData.paymentMethod === "sslcommerz") {
        ...
        const endpoint = couponData
          ? "http://localhost:8000/api/v1/order/withCoupon"
        ...
        const sslResponse = await axios.post(
          "http://localhost:8000/api/v1/order/payment/sslcommerz/initiate",
          { orderId }
        );
        ...
        const bkashResponse = await axios.post(
          "http://localhost:8000/api/v1/order/payment/bkash/initiate",
